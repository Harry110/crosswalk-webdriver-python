...
class RequestCommand():
  ...
  def SetParams(self, params):
    ...
    def run(self):
        ...
        self.command(self.status, self.session_id, self.params, self.value)
...
class ExecuteSessionCommand():
  ...
  def run(self):
    ...
    self.thread.postTask(self.command, self.status, self.session, self.params, self.value, sinal)
    sinal.wait()


def PrepareWindowCommand(command, status, session, web_view, params, value):
...
def DealWindowCommand(command, status, session, web_view, params, value):
